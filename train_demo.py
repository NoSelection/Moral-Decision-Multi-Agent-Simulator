...
class MoralTrainingDemo:
    ...
    def run_training_demo(self, num_episodes=50000, visualize_every=1000):
        ...
        for episode in tqdm(range(num_episodes), desc="🧠 Training Neural Agents"):
            ...
            for agent_id in ["agent_0", "agent_1"]:
                if len(experience_buffers[agent_id]["obs"]) > 32:
                    ...
                    indices = np.random.choice(
                        len(experience_buffers[agent_id]["obs"]), size=recent_size, replace=False
                    )
            ...
            if episode_claims:
                avg_adaptive_claim = (
                    np.mean([claims["agent_0"] + claims["agent_1"] for claims in episode_claims])
                    / 2
                )
    ...
    def _print_progress(self, episode, metrics, rewards):
        ...
        print(
            f"   ⚖️  Fairness Score: {metrics.fairness_score:.3f} ({'🟢 High' if metrics.fairness_score > 0.7 else '🟡 Medium' if metrics.fairness_score > 0.4 else '🔴 Low'})"
        )
        print(
            f"   🤝 Cooperation Index: {metrics.cooperation_index:.3f} ({'🟢 High' if metrics.cooperation_index > 0.6 else '🟡 Medium' if metrics.cooperation_index > 0.3 else '🔴 Low'})"
        )
        print(
            f"   🧠 Adaptive Agent Rewards: A0={rewards['agent_0']:.1f}, A1={rewards['agent_1']:.1f}"
        )
        print(
            f"   🤖 Fixed Agent Rewards: Egoist={rewards['agent_2']:.1f}, Utilitarian={rewards['agent_3']:.1f}"
        )
    ...
    def _run_evaluation(self, env, agents, ggb):
        ...
        for agent_id, resources in final_resources.items():
            agent_type = (
                ...
                else "Egoist" if agent_id == "agent_2" else "Utilitarian"
            )
...
def main():
    ...
    training_history = trainer.run_training_demo(
        num_episodes=NUM_EPISODES, visualize_every=VISUALIZE_EVERY
    )
