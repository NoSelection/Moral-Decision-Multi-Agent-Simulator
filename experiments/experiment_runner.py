...
class MoralExperimentRunner:
    ...
    def __init__(
        ...
        self.experiment_dir = os.path.join(
            save_dir, experiment_name, datetime.now().strftime("%Y%m%d_%H%M%S")
        )
    ...
    def run_experiment(
        ...
        for episode in tqdm(range(self.num_episodes), desc="Episodes"):
            ...
            episode_data = {"rewards": [], "actions": [], "resources": [], "observations": []}

            for step in range(self.episode_length):
                # Get actions
                if use_maddpg and maddpg:
                    actions = maddpg.act(
                        observations, add_noise=(episode < self.num_episodes * 0.8)
                    )
                else:
                    actions = {}
                ...
                if step > 0:
                    influenced = peer_analyzer.detect_influence_event(
                        episode_data["actions"][-2], episode_data["actions"][-1]
                    )
                    if influenced:
                        experiment_data["influence_events"].append(
                            {"episode": episode, "step": step, "influenced_agents": influenced}
                        )
    ...
    def run_peer_pressure_experiment(self) -> Dict:
        ...
        experiments["no_influence"] = self.run_experiment(
            agent_configs=agent_configs, reward_structure="mixed", peer_influence_strength=0.0
        )

        # Experiment 2: Moderate peer influence
        print("\nExperiment 2: Moderate peer influence")
        experiments["moderate_influence"] = self.run_experiment(
            agent_configs=agent_configs, reward_structure="mixed", peer_influence_strength=0.3
        )

        # Experiment 3: Strong peer influence
        print("\nExperiment 3: Strong peer influence")
        experiments["strong_influence"] = self.run_experiment(
            agent_configs=agent_configs, reward_structure="mixed", peer_influence_strength=0.7
        )
    ...
    def run_supervisor_experiment(self) -> Dict:
        ...
        experiments["no_supervisor"] = self.run_experiment(
            agent_configs=agent_configs, reward_structure="mixed", use_maddpg=True
        )
    ...
    def _generate_visualizations(self, experiment_data: Dict, agent_types: Dict):
        ...
        dashboard = self.visualizer.create_interactive_dashboard(
            experiment_data, save_path=os.path.join(self.experiment_dir, "dashboard.html")
        )
...
if __name__ == "__main__":
    ...
    runner = MoralExperimentRunner(
        experiment_name="maebe_moral_dynamics", num_agents=4, episode_length=100, num_episodes=50
    )
